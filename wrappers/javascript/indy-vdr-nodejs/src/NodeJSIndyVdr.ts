...
export class NodeJSIndyVdr implements IndyVdr {
  ...
  private promisifyWithResponse = async <Return>(
    ...
    return new Promise((resolve, reject) => {
      ...
      const registeredCallback = koffi.register(
        (_cbId: number, errorCode: number, response: string) => {
          clearImmediate(keepAlive);
          koffi.unregister(registeredCallback);
  ...
  private getIndyVdrError(errorCode: number): IndyVdrError {
    ...
    if (errorObject.code !== errorCode) {
      return new IndyVdrError({
        code: errorCode,
        message:
          "Error details have already been overwritten on the native side, unable to retrieve error message",
      });
    }
  ...
  public buildGetCredDefRequest(
    ...
    const requestHandle = allocateHandle();
    const { credentialDefinitionId, submitterDid } =
      serializeArguments(options);
  ...
  public buildGetNymRequest(options: GetNymRequestOptions): number {
    const requestHandle = allocateHandle();
    const { dest, submitterDid, seqNo, timestamp } =
      serializeArguments(options);
    const convertedSeqNo = seqNo ?? -1;
    const convertedTimestamp = timestamp ?? -1;
  ...
  public requestSetMultiSignature(
    ...
    const { identifier, requestHandle, signature } =
      serializeArguments(options);
