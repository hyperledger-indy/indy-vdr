...
export const nativeBindings: Record<
  string,
  [koffi.TypeSpec, Array<koffi.TypeSpec>]
> = {
  // configuration
  ...
  indy_vdr_get_current_error: [
    FFI_ERROR_CODE,
    [koffi.out(koffi.pointer(FFI_STRING))],
  ],

  // cache
  indy_vdr_set_ledger_txn_cache: [
    FFI_ERROR_CODE,
    [FFI_INT64, FFI_INT64, FFI_STRING],
  ],
  ...
  indy_vdr_pool_refresh: [
    FFI_ERROR_CODE,
    [FFI_POOL_HANDLE, koffi.pointer(FFI_CALLBACK_NO_RESULT), FFI_CALLBACK_ID],
  ],
  ...
  indy_vdr_request_get_body: [
    FFI_ERROR_CODE,
    [FFI_REQUEST_HANDLE, koffi.out(koffi.pointer(FFI_STRING))],
  ],
  ...
  indy_vdr_request_set_multi_signature: [
    FFI_ERROR_CODE,
    [FFI_REQUEST_HANDLE, FFI_STRING, ByteBufferStruct],
  ],
  indy_vdr_request_set_signature: [
    FFI_ERROR_CODE,
    [FFI_REQUEST_HANDLE, ByteBufferStruct],
  ],
